...
2. **Java 21+ SDK** (для локальной разработки)
3. **Git**
...
2. **Запустите всю инфраструктуру:**
   Эта команда соберет Docker-образ приложения и запустит все необходимые сервисы (`rag-app`, `postgres`, `ollama`,
   `neo4j`), включая автоматическую загрузку LLM-моделей.

   ```bash
   docker-compose up --build -d
   ```

**Внимание:** Первый запуск может занять продолжительное время (10-30 минут в зависимости от скорости сети), так как
сервис `ollama-init` будет скачивать несколько гигабайт весов моделей. Вы можете следить за процессом командой
`docker logs -f rag-ollama-init`.

3. **Проверьте, что все сервисы запущены:**
   После завершения загрузки моделей, проверьте состояние контейнеров:

   ```bash
   docker ps
   ```

    Вы должны увидеть работающие контейнеры `rag-app`, `rag-postgres`, `rag-ollama` и `rag-neo4j`. Затем проверьте health-эндпоинт приложения:
