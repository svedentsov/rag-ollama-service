package com.example.ragollama.agent.executive.api.dto;

import com.example.ragollama.agent.AgentContext;
import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.constraints.Max;
import jakarta.validation.constraints.Min;
import jakarta.validation.constraints.NotNull;

import java.util.Map;

/**
 * DTO для запроса на генерацию стратегического плана по портфелю проектов.
 *
 * @param analysisPeriodDays Период в днях для анализа исторических данных,
 *                           на основе которых будет строиться стратегия.
 */
@Schema(description = "DTO для запроса на генерацию стратегического плана по портфелю проектов")
public record PortfolioStrategyRequest(
        @Schema(description = "Период для анализа исторических данных в днях", defaultValue = "90")
        @NotNull @Min(30) @Max(365)
        Integer analysisPeriodDays
) {
    /**
     * Преобразует DTO в {@link AgentContext} для передачи в конвейер.
     *
     * @return Контекст для запуска агента.
     */
    public AgentContext toAgentContext() {
        return new AgentContext(Map.of("analysisPeriodDays", this.analysisPeriodDays, "days", this.analysisPeriodDays));
    }
}
