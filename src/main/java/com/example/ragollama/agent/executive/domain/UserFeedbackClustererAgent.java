...
public class UserFeedbackClustererAgent implements ToolAgent {
    ...
    public CompletableFuture<AgentResult> execute(AgentContext context) {
        ...
        if (rawFeedback == null || rawFeedback.isEmpty()) {
            return CompletableFuture.completedFuture(new AgentResult(getName(), AgentResult.Status.SUCCESS, "Нет обратной связи для анализа.", Map.of()));
        }
        ...
        try {
            ...
            return CompletableFuture.failedFuture(new ProcessingException("Ошибка сериализации обратной связи.", e));
        }
    }
