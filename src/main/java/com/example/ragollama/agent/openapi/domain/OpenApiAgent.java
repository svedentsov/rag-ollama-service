...
public class OpenApiAgent implements ToolAgent {
    ...
    /**
     * Асинхронно выполняет RAG-запрос к OpenAPI спецификации.
     *
     * @param context Контекст, содержащий {@link OpenApiSourceRequest} и `query`.
     * @return {@link Mono} с результатом, содержащим текстовый ответ.
     * @throws ProcessingException если тип источника в контексте некорректен.
     */
    @Override
    public Mono<AgentResult> execute(AgentContext context) {
        ...
        // Преобразуем Map из UniversalRequest обратно в DTO
        OpenApiQueryRequest request = new OpenApiQueryRequest((OpenApiSourceRequest) sourceObject, query);

        if (!(request.source() instanceof OpenApiSourceRequest)) {
        ...
        return openApiQueryService.querySpec(request.source(), query)
                .map(answer -> new AgentResult(
                        getName(),
