...
public class IndexOptimizerService {
    ...
    public void cleanupStaleDocuments() {
        // ПРИМЕЧАНИЕ: В реальной системе здесь была бы логика сверки с внешним API (Confluence, Jira).
        // Для демонстрации, мы будем имитировать удаление, находя "старые" завершенные задачи.
        // Например, удалим все, что было успешно обработано более 7 дней назад.
        log.info("Поиск и удаление устаревших документов...");
        OffsetDateTime threshold = OffsetDateTime.now().minusDays(7);
        var staleJobs = documentJobRepository.findCompletedJobsBefore(threshold);
        ...
        log.warn("Обнаружено {} устаревших документов для удаления. IDs: {}",
                staleJobs.size(), staleJobs.stream().map(UUID::toString).toList());
    ...
    public void vacuumVectorStore() {
        ...
        try {
            ...
            log.info("Команда VACUUM успешно выполнена.");
        } catch (Exception e) {
