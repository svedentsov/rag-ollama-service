 ...
 * <p>Использует LLM в роли "аудитора" для верификации того, что ответ
...
public class SourceCiteVerifierService {
    ...
    public void verify(List<Document> contextDocuments, String generatedAnswer) {
        ...
        llmClient.callChat(new Prompt(promptString), ModelCapability.FAST_RELIABLE, true)
                .subscribe(
                        tuple -> {
                            String jsonResponse = tuple.getT1();
