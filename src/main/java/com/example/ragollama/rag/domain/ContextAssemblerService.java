 ...
 * <p>
 * Этот сервис решает две ключевые задачи:
 * 1. Применяет выбранную {@link ContextArrangementStrategy} для оптимального
 * расположения документов.
 * 2. Управляет контекстным окном, отбирая столько документов из
 * упорядоченного списка, сколько помещается в заданный лимит токенов.
 */
@Service
@Slf4j
