 ...
 * <p> Каждая реализация инкапсулирует отдельный фактор, влияющий на финальный
 * порядок документов (например, совпадение по ключевым словам, новизна, авторитетность источника).
 * Стратегии применяются последовательно и могут модифицировать оценку релевантности
...
public interface RerankingStrategy {
     ...
     * <p> Стратегия должна прочитать существующую оценку (например, 'rerankedSimilarity'),
