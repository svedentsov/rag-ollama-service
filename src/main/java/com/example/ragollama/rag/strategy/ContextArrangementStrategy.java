...
/**
 * Интерфейс для стратегий, отвечающих за переупорядочивание и/или
 * фильтрацию списка документов перед их сборкой в финальный контекст.
 * <p>
 * Реализации этого интерфейса позволяют гибко управлять тем, в каком
 * порядке и какие именно документы будут представлены LLM, что является
 * ключевым фактором для повышения качества генерации.
 */
@FunctionalInterface
public interface ContextArrangementStrategy {
     ...
     *                  по убыванию релевантности от векторного поиска.
