...
import com.example.ragollama.qaagent.model.TestOracleReport;
...
public class XaiTestOracleAgent implements ToolAgent {
    ...
    public CompletableFuture<AgentResult> execute(AgentContext context) {
        ...
        if (generatedTests.isEmpty()) {
            return CompletableFuture.completedFuture(new AgentResult(getName(), AgentResult.Status.SUCCESS, "Нет тестов для анализа.", Map.of()));
        }
        ...
        try {
            ...
            return CompletableFuture.failedFuture(new ProcessingException("Ошибка сериализации сгенерированных тестов", e));
        }
    }
